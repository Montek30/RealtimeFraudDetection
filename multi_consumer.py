...
def send_mail(to_email, topic):
	login, password = 'monteks764@gmail.com', "heyhey123@H"
	recipients = [to_email]

	msg = MIMEText('Alert! Fraudulent Transaction has been detected from your card on topic' + topic, 'plain', 'utf-8')
...
def consumeData(topic):
	...
	try:
		...
		consumer = KafkaConsumer(topic , group_id='my-tenth-application',bootstrap_servers=['localhost:9092'], api_version=(0,11,5))
	except:
		print("Error!!")
	...
	for msg in consumer:
		...
		df=df.withColumn('i',df['i'].cast("double").alias('i'))
...
try:
    t1 = Process(target=consumeData, args=('tc_1',))
    t2 = Process(target=consumeData, args=('tc_2',))
    t1.start()
    t2.start()
except:
   print ("Error: unable")